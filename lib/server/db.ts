...
export function findTestBySlug(slug: string): DbTest | undefined {
  // First try to find by publicSlug
  const db = load()
  const test = db.tests.find((t) => (t as DbTest & { publicSlug?: string }).publicSlug === slug)
  if (test) return test
  // Fallback: find link by token and return associated test
  const link = db.links.find((l) => l.token === slug)
  if (link) return db.tests.find((t) => t.id === link.testId)
  return undefined
}
...
export function updateAttemptAnswers(attemptId: string, answers: { questionId: string; value: unknown }[]) {
...
export function seedIfEmpty() {
  ...
  if (db.users.length === 0) {
    ...
    // Ensure demo accounts have correct passwords (fix for corrupted hashes)
    const adminUser = db.users.find(u => u.email === 'admin@admin.com')
    if (adminUser && !verifyPassword('admin123', adminUser.passwordHash)) {
      adminUser.passwordHash = hashPassword('admin123')
      persist()
    }
    const psychoUser = db.users.find(u => u.email === 'psycho@psycho.com')
    if (psychoUser && !verifyPassword('psycho123', psychoUser.passwordHash)) {
      psychoUser.passwordHash = hashPassword('psycho123')
      persist()
    }
  }
...
// Function to reset a user's password (useful for debugging)
